<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['accessibilityId','transportationType','isAccessible','assistancesAvailable','language',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="accessibilityId" header="accessibilityId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="transportationType" header="transportationType" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="isAccessible" header="isAccessible" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="assistancesAvailable" header="assistancesAvailable" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="language" header="language" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">AccessibilityInfo 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <AccessibilityInfo :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="accessibilityInfoModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
<script>
import AccessibilityInfo from '../components/AccessibilityInfo.vue'
...
export default {
    name : 'accessibilityInfo-grid',
    mixins:[AccessibilityInfo],
    components:{
        AccessibilityInfo,
    },
    data: () => ({
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.accessibilityInfo.href;
